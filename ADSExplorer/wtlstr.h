// Standalone version of CString for ATL
// code is taken from WTL 7.0, see copyright notice below
//
// This permits to use CString in ATL, note that it is intended
// for ATL 3.0, ATL 7.0 has native CString support.
...
#if !defined(__WTLSTR_H__) && !defined(__ATLMISC_H__) && \
	!defined(__ATLSTR_H__) && !defined(__AFXSTR_H__)
...
_declspec(selectany) LPCTSTR _atltmpPchNil =
   ...
   protected:
...
#endif	//_DEBUG
		int nLen =
			::LoadString(_Module.GetResourceInstance(), nID, lpszBuf, nMaxBuf);
		if (nLen == 0) {
			lpszBuf[0] = '\0';
		}
		return nLen;
	...
	static int __cdecl _wcstombsz(
		...
		int result = ::WideCharToMultiByte(
			CP_ACP, 0, wcstr, -1, mbstr, (int) count, NULL, NULL
		);
	...
	static int __cdecl _mbstowcsz(
		...
		int result =
			::MultiByteToWideChar(CP_ACP, 0, mbstr, -1, wcstr, (int) count);
...
// Helpers to avoid CRT startup code
	...
	static int _cstrcoll(const TCHAR *pstrOne, const TCHAR *pstrOther) {
		int nRet =
			CompareString(GetThreadLocale(), 0, pstrOne, -1, pstrOther, -1);
		ATLASSERT(nRet != 0);
		return nRet -
			   2;  // Convert to strcmp convention.  This really is documented.
	}
	static int _cstrcolli(const TCHAR *pstrOne, const TCHAR *pstrOther) {
		int nRet = CompareString(
			GetThreadLocale(), NORM_IGNORECASE, pstrOne, -1, pstrOther, -1
		);
		ATLASSERT(nRet != 0);
		return nRet -
			   2;  // Convert to strcmp convention.  This really is documented.
...
#else	//!_ATL_MIN_CRT
	static TCHAR *_cstrchr(const TCHAR *p, TCHAR ch) { return _tcschr(p, ch); }
	static TCHAR *_cstrrchr(const TCHAR *p, TCHAR ch) {
		return _tcsrchr(p, ch);
	}
	static TCHAR *_cstrrev(TCHAR *pStr) { return _tcsrev(pStr); }
	static TCHAR *_cstrstr(const TCHAR *pStr, const TCHAR *pCharSet) {
	...
	static int _cstrisdigit(TCHAR ch) { return _istdigit(ch); }
	static int _cstrisspace(TCHAR ch) { return _istspace(ch); }
	static int _cstrcmp(const TCHAR *pstrOne, const TCHAR *pstrOther) {
	...
	static int _cstrtoi(const TCHAR *nptr) { return _ttoi(nptr); }
#endif	//!_ATL_MIN_CRT
};
...
inline CString::operator LPCTSTR() const { return m_pchData; }
...
// assumes [optimistically] that data length will equal allocation length
{
	...
	if (nLen == 0) {
		...
		ATLTRY(
			pData = (CStringData *) new BYTE
				[sizeof(CStringData) + (nLen + 1) * sizeof(TCHAR)]
		);
		if (pData == NULL) {
			return FALSE;
		}
...
inline void CString::AllocCopy(
	...
	if (nNewLen == 0) {
		...
		if (dest.AllocBuffer(nNewLen)) {
			memcpy(
				dest.m_pchData, m_pchData + nCopyIndex, nCopyLen * sizeof(TCHAR)
			);
		}
	}
}
...
inline BOOL CString::ConcatCopy(
	int nSrc1Len,
	LPCTSTR lpszSrc1Data,
	int nSrc2Len,
	LPCTSTR lpszSrc2Data
	...
	if (nNewLen != 0) {
		...
		if (bRet) {
			...
			memcpy(
				m_pchData + nSrc1Len, lpszSrc2Data, nSrc2Len * sizeof(TCHAR)
			);
		}
	}
	return bRet;
...
inline CString __stdcall
operator+(const CString &string1, const CString &string2) {
	CString s;
	s.ConcatCopy(
		string1.GetData()->nDataLength,
		string1.m_pchData,
...
inline void CString::ConcatInPlace(int nSrcLen, LPCTSTR lpszSrcData) {
	...
	if (GetData()->nRefs > 1 ||
		...
		if (ConcatCopy(
				GetData()->nDataLength, m_pchData, nSrcLen, lpszSrcData
			)) {
	...
	if (GetData()->nDataLength != GetData()->nAllocLength) {
		...
		if (AllocBuffer(GetData()->nDataLength)) {
			memcpy(
				m_pchData,
				pOldData->data(),
				pOldData->nDataLength * sizeof(TCHAR)
			);
...
inline int CString::Find(TCHAR ch) const { return Find(ch, 0); }
	...
	if (nLength != 0) {
		if (AllocBuffer(nLength)) {
			int n = ::MultiByteToWideChar(
				CP_ACP, 0, lpsz, nLength, m_pchData, nLength + 1
			);
...
inline int CString::Find(LPCTSTR lpszSub) const { return Find(lpszSub, 0); }
		...
		if (nItemLen != 0) {
			...
			switch (*lpsz) {
...
#ifndef _DEBUG
					::OutputDebugString(
						_T("Floating point (%%e, %%f, %%g, and %%G) is not ")
						_T("supported by the WTL::CString class.")
					);
					::DebugBreak();
#endif	//!_DEBUG
					break;
				...
				case 'f': {
					...
					pszTemp =
						(LPTSTR) _alloca(max(nWidth, 312 + nPrecision + 6));

					f = va_arg(argList, double);
					_stprintf(
						pszTemp, _T( "%*.*f" ), nWidth, nPrecision + 6, f
					);
...
inline void CString::TrimLeft() {
	...
	int nDataLength =
		GetData()->nDataLength - (int) (DWORD_PTR) (lpsz - m_pchData);
...
inline int CString::Insert(int nIndex, TCHAR ch) {
	...
	memmove(
		m_pchData + nIndex + 1,
		m_pchData + nIndex,
		(nNewLength - nIndex) * sizeof(TCHAR)
	);
	m_pchData[nIndex] = ch;
	GetData()->nDataLength = nNewLength;
...
inline int CString::Insert(int nIndex, LPCTSTR pstr) {
	...
	if (nInsertLength > 0) {
		...
		if (GetData()->nAllocLength < nNewLength) {
			...
			memcpy(
				m_pchData, pstr, (pOldData->nDataLength + 1) * sizeof(TCHAR)
			);
			CString::Release(pOldData);
		}
		...
		// move existing bytes down
		memmove(
			m_pchData + nIndex + nInsertLength,
			m_pchData + nIndex,
	...
	if (nCount > 0) {
		...
		while (lpszStart < lpszEnd) {
			while ((lpszTarget = _cstrstr(lpszStart, lpszOld)) != NULL) {
				int nBalance =
					nOldLength -
					((int) (DWORD_PTR) (lpszTarget - m_pchData) + nSourceLen);
				memmove(
...
inline BSTR CString::AllocSysString() const {
	...
	int nLen = MultiByteToWideChar(
		CP_ACP, 0, m_pchData, GetData()->nDataLength, NULL, NULL
	);
	BSTR bstr = ::SysAllocStringLen(NULL, nLen);
	if (bstr != NULL) {
		MultiByteToWideChar(
			CP_ACP, 0, m_pchData, GetData()->nDataLength, bstr, nLen
		);
	}
#endif
	return bstr;
...
inline BSTR CString::SetSysString(BSTR *pbstr) const {
	...
	if (::SysReAllocStringLen(pbstr, NULL, nLen)) {
		MultiByteToWideChar(
			CP_ACP, 0, m_pchData, GetData()->nDataLength, *pbstr, nLen
		);
	}
#endif
...
#endif	// !defined(__WTLSTR_H__) && !defined(__ATLMISC_H__) &&
		// !defined(__ATLSTR_H__) && !defined(__AFXSTR_H__)
